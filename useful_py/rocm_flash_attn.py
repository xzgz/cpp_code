...
def filter_dump(root_data_dir, skip_data_tensor_name_list, **kwargs):
    ...
    if layer_id == 0:
        if decoder_step_id % decoder_skip_step_count == 0:
            # if sample_id < 25:
            if sample_id in select_sample_id:
...
class ROCmFlashAttentionImpl(AttentionImpl):
    ...
    def forward(
        ...
        * For cross-attention:
        ...
        filter_print(f"global_count={global_count}")
        filter_print(f"query.dtype={query.dtype}")
        filter_print(f"key.dtype={key.dtype}")
        filter_print(f"kv_cache.dtype={kv_cache.dtype}")
        filter_print(f"query.shape={query.shape}")
        filter_print(f"key.shape={key.shape}")
        filter_print(f"kv_cache.shape={kv_cache.shape}")
        ...
        filter_print(f"AAAA=self.attn_type={self.attn_type}, kv_cache.numel()={kv_cache.numel()}")
        if self.attn_type not in [
                AttentionType.ENCODER, AttentionType.ENCODER_ONLY
        ] and kv_cache.numel() > 0:
            filter_print(f"AAAA=split_kv_cache")
            key_cache, value_cache = paged_attn.split_kv_cache(
                kv_cache, self.num_kv_heads, self.head_size)

            if key is not None and value is not None:
                filter_print(f"AAAA=write_to_paged_cache")
        ...
        if prefill_meta := attn_metadata.prefill_metadata:
            filter_print(f"AAAA=prefill_meta is True")
            # Prompt run.
            ...
            if kv_cache.numel() == 0 or prefill_meta.block_tables.numel() == 0:
                filter_print(f"AAAA=kv_cache.numel() == 0 or prefill_meta.block_tables.numel() == 0")
                # triton attention
                # When block_tables are not filled, it means q and k are the
                # prompt, and they have the same length.
                attn_masks = None
                if self.use_triton_flash_attn:
                    filter_print(f"AAAA=use_triton_flash_attn")
                    ...
                    filter_print(f"AAAA=use_naive_attn")
                ...
                filter_print(f"AAAA=kv_cache.numel() != 0 and prefill_meta.block_tables.numel() != 0")
        ...
        if (decode_meta := attn_metadata.decode_metadata) and (
            ...
            filter_print(f"AAAA=decode_meta is True and self.attn_type != AttentionType.ENCODER_ONLY")
            # Decoding run.
            ...
            if use_custom:
                filter_print(f"AAAA=use_custom")
                ...
                filter_print(f"AAAA=use_front={self.attn_type != AttentionType.ENCODER_DECODER}")
                filter_print(f"decode_meta.seq_lens_tensor={decode_meta.seq_lens_tensor}")
                filter_print(f"decode_meta.encoder_seq_lens_tensor={decode_meta.encoder_seq_lens_tensor}")
                ...
                # root_data_dir = "/lab-mlperf-inference/code/op_paged_attention_rocm_input/llama2_70b/dk2_eager_step1"
                ...
                filter_print(f"AAAA=is_rocm_aiter_paged_attn_enabled")
                ...
                root_data_dir = "/lab-mlperf-inference/code/op_pa_fwd_asm_input/llama2_70b/dk2_eager_step11"
                # skip_data_tensor_name_list = ["tmp_out", "query", "key_cache", "value_cache"]
                skip_data_tensor_name_list = []
                filter_dump(
                    root_data_dir,
                ...
                filter_print(f"AAAA=NOT is_rocm_aiter_paged_attn_enabled")
